...
export function SaveButton({ 
  ...
  const buttonVariants = unstyled
    ? {
        idle: { scale: 1 },
        saving: { scale: 1 },
        saved: { scale: [1, 1.05, 1], transition: { duration: 0.2, times: [0, 0.5, 1] } },
      }
    : {
        idle: {
  ...
  return (
    <div className="relative">
      <motion.button
        ...
        className={cn(
          unstyled ? "" : "group relative grid overflow-hidden rounded-full px-6 py-2 transition-all duration-200",
          !unstyled && status === "idle"
            ? "shadow-[0_1000px_0_0_hsl(0_0%_85%)_inset] dark:shadow-[0_1000px_0_0_hsl(0_0%_20%)_inset]"
            : "",
          !unstyled && "hover:shadow-lg",
          className
        )}
      ...
      >
        ...
        />)}
        <span className={cn("z-10 flex items-center justify-center gap-2 text-sm font-medium", contentClassName)}>
          {icon}
      ...
      {overlayVisible && (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/40">
          <div className="flex items-center gap-3 rounded-full bg-white/10 px-4 py-2 backdrop-blur-md border border-white/20">
